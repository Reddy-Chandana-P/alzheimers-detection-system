    ...
    if (result.lime && result.shap) {
        html += `
            <div class="explainability-section">
                ...
                <div style="margin: 30px 0; padding: 20px; background: rgba(0,229,160,0.05); border-radius: 10px; border: 1px solid rgba(0,229,160,0.2);">
                    <h4 style="color: #00e5a0; margin-bottom: 10px;">🔀 Counterfactual Explanation</h4>
                    <p class="explanation-text">
                    ...
                    <p class="explanation-text" style="margin-top: 8px;">
                        <strong>How it works:</strong> Starting from the original image, the system iteratively applies tiny gradient-based pixel modifications that push the model's prediction toward the next most likely class, while minimizing the total change to the image. It stops as soon as the prediction flips.
                    </p>
                    ...
                    <p class="explanation-text" style="margin-top: 15px; font-size: 0.9rem; opacity: 0.8;">
                        <strong>How to read the panels:</strong> Left = original scan with current diagnosis. Middle = change map (brighter = more pixels modified). Right = counterfactual scan with the new diagnosis. A small percentage of changed pixels means the two diagnoses are close — the model is near the decision boundary.
                    </p>
                    <p class="explanation-text" style="margin-top: 10px; font-size: 0.9rem; opacity: 0.8;">
                        <strong>Clinical value:</strong> The change map highlights which brain regions are most critical for distinguishing between the two diagnoses — typically the hippocampus and temporal lobes in Alzheimer's staging. A high-confidence prediction should require large changes to flip; if it flips easily, the model is near the decision boundary and the case warrants closer clinical review.
                    </p>
                </div>
                ` : ''}
